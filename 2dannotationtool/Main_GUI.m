function varargout = Main_GUI(varargin) %#ok<*DEFNU> % A. I. mod 10.8.2023
...
handles.data_processing_information.annotation_status = studies_table.AnnotationStatus; % 0: Pending, 1: Incomplete, 2: Ready
...
handles.data_processing_information.study_index = 1; % starting with first study on the list
...
ViewStudiesGUI('update_and_load_status', handles)
...
set(handles.uipanel_study,'Visible','off') % FIXME: replace 'off' with 'on' if you prefer to type the study id by hand instead of selecting it from the study list GUI
set(handles.menu_item_save,'Enable','on')
set(handles.menu_view,'Visible','on')
set(handles.menu_study,'Visible','on')
set(handles.menu_item_study_list,'Visible','on')
...
if ~isfield(handles,'current_study') % A. I. mod 8.4.2020
    return;
end
...
if strcmp(eventdata.EventName,'WindowKeyPress')
    if strcmp(eventdata.Key,'leftarrow') && handles.data_processing_information.annotation_status(current_study_index - 1) == 2
        study_idx_unannotated = find(handles.data_processing_information.annotation_status == 0 | handles.data_processing_information.annotation_status == 1); % studies which are not marked as ready, i.e. "2"
        idxs = study_idx_unannotated(study_idx_unannotated < (current_study_index - 1));
        if isempty(idxs)
            uiwait(msgbox('There is no previous pending or incomplete study.','Previous','help'));
            return;
        end
    elseif strcmp(eventdata.Key,'rightarrow') && handles.data_processing_information.annotation_status(current_study_index + 1) == 2
        study_idx_unannotated = find(handles.data_processing_information.annotation_status == 0 | handles.data_processing_information.annotation_status == 1); % studies which are not marked as ready, i.e. "2"
        idxs = study_idx_unannotated(study_idx_unannotated > (current_study_index + 1));
        if isempty(idxs)
            uiwait(msgbox('There is no next pending or incomplete study.','Next','help'));
            return;
        end
    end
end
...
status = handles.data_processing_information.annotation_status(current_study_index);
handles.data_processing_information.annotation_status(current_study_index) = status;
ViewStudiesGUI('update_and_load_status', handles, true)
...
elseif ~isempty(user_input_study_idx) && strcmp(eventdata.EventName,'CellSelection')
    handles.data_processing_information.study_index = user_input_study_idx;
end

if strcmp(eventdata.EventName,'WindowKeyPress')
    % Check if annotation status is "2" (i.e., "Ready") and step forward
    if handles.data_processing_information.annotation_status(handles.data_processing_information.study_index) == 2
        study_idx_unannotated = find(handles.data_processing_information.annotation_status == 0 | handles.data_processing_information.annotation_status == 1); % studies which are not marked as ready, i.e. "2"
        if strcmp(eventdata.Key,'leftarrow')
...
function pushbutton_jump_Callback(hObject, eventdata, handles)  % FIXME: Currently disabled
...
% --------------------------------------------------------------------
function menu_item_study_list_Callback(hObject, eventdata, handles) %#ok<INUSD>
% hObject    handle to menu_item_study_list (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% View ViewStudiesGUI
if ~isempty(findobj('Tag','studies_view'))
    return;
else
    ViewStudiesGUI;
    ViewStudiesGUI('update_and_load_status', handles)

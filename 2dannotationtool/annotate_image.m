...
roi = drawfreehand(handles_view.axes_mammogram,'Color',line_color,'Smoothing',1,'FaceAlpha',0,'DrawingArea',[0,0,w,h],'LabelVisible','hover','Label',label,'InteractionsAllowed','reshape'); % A. I. mod 11.3.2020

% Handle the situation when ROI creation is aborted
try
    % Create mask, see https://www.mathworks.com/help/images/ref/createmask.html
    new_segmentation_mask = createMask(roi, img); % A. I. mod
    
    % Delete ROI object
    delete(roi);
    
    new_segmentation_mask = uint16(new_segmentation_mask);
    [y, x] = find(new_segmentation_mask); % A. I. mod 15.3.2020
    x_center = mean(x); % A. I. mod 15.3.2020
    y_center = mean(y); % A. I. mod 15.3.2020
    ...
    if strcmp(label,'malignant_mass')
        ...
        mask_char = get_characterizing_input(label, handles_view); % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_mass_char(val) = mask_char; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_mass_x_center(val) = x_center; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_mass_y_center(val) = y_center; % A. I. mod 15.3.2020
    elseif strcmp(label,'benign_mass')
        ...
        mask_char = get_characterizing_input(label, handles_view); % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_mass_char(val) = mask_char; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_mass_x_center(val) = x_center; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_mass_y_center(val) = y_center; % A. I. mod 15.3.2020
    elseif strcmp(label,'malignant_calc')
        ...
        mask_char = get_characterizing_input(label, handles_view); % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_calc_char(val) = mask_char; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_calc_x_center(val) = x_center; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_calc_y_center(val) = y_center; % A. I. mod 15.3.2020
    elseif strcmp(label,'benign_calc')
        ...
        mask_char = get_characterizing_input(label, handles_view); % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_calc_char(val) = mask_char; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_calc_x_center(val) = x_center; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_calc_y_center(val) = y_center; % A. I. mod 15.3.2020
    elseif strcmp(label,'malignant_architechtural_distortion') % A. I. mod 18.3.2020
        ...
        mask_char = get_characterizing_input(label, handles_view); % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_architechtural_distortion_char(val) = mask_char; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_architechtural_distortion_x_center(val) = x_center; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_malignant_architechtural_distortion_y_center(val) = y_center; % A. I. mod 15.3.2020
    elseif strcmp(label,'benign_architechtural_distortion') % A. I. mod 18.3.2020
        ...
        mask_char = get_characterizing_input(label, handles_view); % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_architechtural_distortion_char(val) = mask_char; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_architechtural_distortion_x_center(val) = x_center; % A. I. mod 15.3.2020
        handles_main.out(image_idx).annotation_benign_architechtural_distortion_y_center(val) = y_center; % A. I. mod 15.3.2020
    end
catch
    % do no-op
end
